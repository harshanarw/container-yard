# Dropbox Cloud Storage — Setup Guide (CYMS)

How to connect a Container Yard Management System (CYMS) installation to Dropbox
...
- **Each CYMS installation stores its documents through one active provider.** The
...
- **One shared Dropbox app, one folder per company.** We use a single
  Dropbox **"App folder"** app named **CYMS**. Dropbox sandboxes it to
  `/Apps/CYMS/`. Each company installation writes into its **own subfolder** of
  that app folder, chosen by the **Root Folder** setting in the Yard system:

  ```
  Dropbox
  └── Apps/
      └── CYMS/                ← created & owned by the CYMS app (one App Key)
...
- **Why one app (not one per company):** with "App folder" scope you cannot nest
  ...
  own `/Apps/<AppName>/`. To get the browsable "open CYMS → see every company"
  layout, all companies share the one **CYMS** app key and are separated by their
...
4. **Name your app:** type **`CYMS`**.
   - App names are **globally unique across all Dropbox developers.** If `CYMS`
     is taken, choose another (e.g. `CYMS-YourGroup`). **The name becomes the
...
> The folder `/Apps/CYMS/` is **not** created now. Dropbox creates it the first
> time a company connects (Part B). Do **not** pre-create it by hand — a manual
> folder with the same name causes Dropbox to make `CYMS (1)` instead.
...
3. Fill in the fields:
   - **App Key** — the CYMS app's key (same for all companies).
   - **App Secret** — the CYMS app's secret (same for all companies).
   - **Root Folder** — this company's **unique subfolder**, e.g. `/Acme`.
     - Do **not** use `/CYMS` — you are already inside the CYMS app folder.
     - Change it from the default `/container-yard` to the company name.
     - Result in Dropbox: `/Apps/CYMS/Acme/…`.
...
5. Click **Connect with Dropbox** → you're redirected to Dropbox → **Allow**.
   - Authorize with the account that owns the CYMS app.
...
7. Click **Test Connection** → writes and deletes a temporary file in the
   ...
   subfolder**, so `/Apps/CYMS/Acme/` now exists.
...
1. In the Yard system, upload a document (e.g. a survey photo or an estimate
   attachment).
2. In Dropbox, open `/Apps/CYMS/<Company>/` and confirm the file appears. Paths
   look like:

   ```
   /Apps/CYMS/Acme/surveys/42/9fK3a1..._front-panel.jpg
   ```
...
**Dropbox side (reuse the one CYMS app):**
...
- [ ] Settings → Cloud Storage → Dropbox:
      **App Key / Secret** = CYMS app's, **Root Folder** = `/NewCompanyName`.
- [ ] Save Configuration → Connect with Dropbox → Save & Activate → Test
      Connection.
- [ ] Verify a test upload lands in `/Apps/CYMS/NewCompanyName/`.
...
| Files appear in `/Apps/CYMS/CYMS/…` or `/Apps/CYMS/container-yard/…` | Root Folder set to `/CYMS` or left at default | Set Root Folder to just the company name, e.g. `/Acme`. |
| Two folders: `CYMS` and `CYMS (1)` | A manual folder existed before the app created its own | Delete the empty manual folder; let the app own `/Apps/CYMS/`. |
