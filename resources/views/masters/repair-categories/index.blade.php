...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item active">Repair Categories</li>
@endsection
...
<div class="page-header d-flex align-items-center justify-content-between mb-4">
    <div>
        <h4><i class="bi bi-tags me-2 text-primary"></i>Repair Categories</h4>
        <p class="text-muted small mb-0">Define work order grouping categories. Each work order covers one category.</p>
    </div>
...
<div class="card">
        ...
        <table class="table table-hover align-middle mb-0">
            <thead class="table-light">
                <tr>
                    ...
                    <th>Name</th>
                    <th>Description</th>
                    <th style="width:110px;">Badge Preview</th>
            ...
            @forelse($categories as $cat)
                <tr data-id="{{ $cat->id }}" class="{{ $cat->is_active ? '' : 'table-secondary text-muted' }}">
                    ...
                    <td class="small text-muted fw-semibold">{{ $cat->sort_order }}</td>
                    <td>
            ...
            @empty
                <tr>
                    <td colspan="8" class="text-center text-muted py-5">
                        ...
                        No repair categories yet. Click <strong>Add Category</strong> to get started.
                    </td>
                </tr>
            @endforelse
            </tbody>
    ...
    <div class="card-footer bg-white py-2">
        <span class="text-muted small">
            {{ $categories->count() }} categories · {{ $categories->where('is_active', true)->count() }} active
        </span>
    </div>
</div>
...
<div class="modal fade" id="addModal" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            <form method="POST" action="{{ route('masters.repair-categories.store') }}">
                @csrf
                <div class="modal-header border-0 pb-0">
                    <h6 class="modal-title"><i class="bi bi-plus-circle me-1 text-primary"></i>Add Repair Category</h6>
                    ...
                    <div class="row g-2 mb-3">
                        <div class="col-4">
                            ...
                            <input type="text" name="code" class="form-control text-uppercase font-monospace"
                                   maxlength="10" required placeholder="e.g. STR"
                                   oninput="this.value=this.value.toUpperCase()">
                        </div>
                        <div class="col-8">
                            <label class="form-label fw-semibold">Name <span class="text-danger">*</span></label>
                            <input type="text" name="name" class="form-control" maxlength="100" required placeholder="e.g. Structural Repairs">
                        </div>
                    </div>
                    <div class="mb-3">
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <form method="POST" id="editForm">
                @csrf @method('PATCH')
                <div class="modal-header border-0 pb-0">
                    <h6 class="modal-title"><i class="bi bi-pencil me-1 text-primary"></i>Edit Repair Category</h6>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body">
                    <div class="row g-2 mb-3">
    ...
    <div class="modal-dialog modal-sm">
        <div class="modal-content">
            ...
            <div class="modal-body pt-1">
                <p class="small mb-0">Delete category <strong id="deleteLabel"></strong>?
                   Work orders using this category will lose the reference.</p>
            </div>
            <div class="modal-footer border-0 pt-0">
                <button type="button" class="btn btn-sm btn-secondary" data-bs-dismiss="modal">Cancel</button>
...
if (tbody) {
    Sortable.create(tbody, {
        ...
        onEnd() {
            ...
            fetch('{{ route('masters.repair-categories.reorder') }}', {
                method: 'POST',
...
document.querySelectorAll('.btn-edit').forEach(btn => {
    btn.addEventListener('click', () => {
        ...
        document.getElementById('editColor').value       = btn.dataset.color ?? 'secondary';
        document.getElementById('editForm').action       = '/masters/repair-categories/' + btn.dataset.id;
...
document.querySelectorAll('.btn-delete').forEach(btn => {
    btn.addEventListener('click', () => {
        ...
        document.getElementById('deleteForm').action       = '/masters/repair-categories/' + btn.dataset.id;
