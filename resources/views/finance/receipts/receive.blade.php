...
<div class="page-header d-flex justify-content-between align-items-center">
    <div>
        <h4><i class="bi bi-cash-coin me-2 text-primary"></i>Receive Payment</h4>
        <p class="text-muted mb-0 small">Select a customer, tick the invoices being paid, then save or post the receipt.</p>
    </div>
...
{{-- Step 1 — choose the customer (GET reload) --}}
<div class="card content-card mb-3">
    <div class="card-body">
        <form method="GET" action="{{ route('finance.receipts.receive') }}" class="row g-2 align-items-end">
            <div class="col-md-6">
                ...
                <select name="customer_id" class="form-select form-select-sm s2-code" data-s2-sel="name"
                    ...
                    <option value="{{ $c->id }}"
                        ...
                        {{ (string) optional($customer)->id === (string) $c->id ? 'selected' : '' }}>
                        {{ $c->name }}
                    </option>
                    @endforeach
                </select>
            ...
            <div class="col-md-3">
                <button type="submit" class="btn btn-sm btn-outline-primary"><i class="bi bi-search me-1"></i>Load invoices</button>
            </div>
        </form>
    </div>
</div>
...
@if($customer)
    ...
    <div class="card content-card">
        <div class="card-body text-center text-muted py-5">
            ...
            No outstanding invoices for <strong>{{ $customer->name }}</strong>.
        </div>
    </div>
    @else
    {{-- Step 2 — receipt details + invoice selection --}}
    <form method="POST" action="{{ route('finance.receipts.receive.store') }}" id="receiveForm">
        @csrf
            ...
            <div class="card-body">
                <div class="row g-3">
                    <div class="col-md-3">
                        ...
                        <input type="date" name="receipt_date" class="form-control form-control-sm" value="{{ old('receipt_date', date('Y-m-d')) }}" required>
                    </div>
                    <div class="col-md-3">
                    ...
                    <div class="col-12">
                        ...
                        <input type="text" name="narration" class="form-control form-control-sm" value="{{ old('narration', 'Payment received from '.$customer->name) }}" required maxlength="255">
                    </div>
                </div>
            </div>
        </div>
                ...
                <table class="table table-sm align-middle mb-0 small">
                    ...
                    <tbody>
                        ...
                        <tr data-currency="{{ $rowCcy }}">
                            ...
                            <td>
                                <input type="hidden" name="allocations[{{ $i }}][type]" value="{{ $pi['type'] }}">
...
(function () {
    ...
    // Auto exchange rate from the master.
