...
@section('title', 'AP Credit Note ' . $apCreditNote->credit_note_no)

@section('breadcrumb')
    <li class="breadcrumb-item">Finance</li>
    <li class="breadcrumb-item"><a href="{{ route('finance.ap-credit-notes.index') }}">AP Credit Notes</a></li>
    <li class="breadcrumb-item active">{{ $apCreditNote->credit_note_no }}</li>
@endsection

@section('content')
@php $b = \App\Models\ApCreditNote::statusBadge($apCreditNote->status); @endphp
...
<div class="page-header d-flex justify-content-between align-items-center">
    ...
    <div class="d-flex gap-2">
        ...
        <form method="POST" action="{{ route('finance.ap-credit-notes.approve', $apCreditNote) }}" onsubmit="return confirm('Approve and post credit note {{ $apCreditNote->credit_note_no }} to GL?')">
            @csrf<button class="btn btn-success btn-sm"><i class="bi bi-check2-circle me-1"></i>Approve &amp; Post</button>
        </form>
        @elseif($apCreditNote->isApproved())
        <form method="POST" action="{{ route('finance.ap-credit-notes.cancel', $apCreditNote) }}" onsubmit="return confirm('Cancel this credit note? The GL journal will be reversed and any applications removed.')">
        ...
        @endif
        @endcan
        @can('finance.ap-credit-notes.pdf')
        <a href="{{ route('finance.ap-credit-notes.pdf', $apCreditNote) }}" target="_blank" class="btn btn-outline-secondary btn-sm"><i class="bi bi-printer me-1"></i>Print A4</a>
        <a href="{{ route('finance.ap-credit-notes.pdf', ['apCreditNote' => $apCreditNote, 'size' => 'half']) }}" target="_blank" class="btn btn-outline-secondary btn-sm"><i class="bi bi-file-earmark-text me-1"></i>Half Page</a>
        @endcan
        @can('finance.ap-credit-notes.email')
...
<div class="modal fade" id="emailModal" tabindex="-1"><div class="modal-dialog"><div class="modal-content">
    <form method="POST" action="{{ route('finance.ap-credit-notes.email', $apCreditNote) }}" id="emailForm">
        @csrf
        <div class="modal-header"><h6 class="modal-title"><i class="bi bi-envelope me-1 text-primary"></i>Email Credit Note {{ $apCreditNote->credit_note_no }}</h6><button class="btn-close" data-bs-dismiss="modal"></button></div>
        <div class="modal-body">
            <div class="mb-2"><label class="form-label small fw-semibold">To <span class="text-danger">*</span></label>
                <input type="email" name="to_email" class="form-control form-control-sm" required value="{{ $apCreditNote->supplier->email ?? '' }}"></div>
        ...
        <div class="card content-card h-100">
            ...
            <div class="card-body">
                <table class="table table-sm mb-0 small">
                    ...
                    <tr><td class="text-muted">Applied / Unapplied</td><td class="text-end font-monospace">{{ number_format($apCreditNote->applied_total,2) }} / <span class="fw-semibold">{{ number_format($apCreditNote->unapplied,2) }}</span></td></tr>
                    @if($apCreditNote->reason)<tr><td class="text-muted">Reason</td><td>{{ $apCreditNote->reason }}</td></tr>@endif
                    @if($apCreditNote->journal)<tr><td class="text-muted">GL Journal</td><td class="font-monospace">{{ $apCreditNote->journal->journal_no ?? ('#'.$apCreditNote->journal_id) }}</td></tr>@endif
                </table>
            </div>
        </div>
    </div>
    ...
    <div class="col-md-7">
        <div class="card content-card h-100">
            ...
            <div class="table-responsive">
                <table class="table table-sm mb-0 small">
                    <thead class="table-light"><tr><th>Description</th><th>Expense Account</th><th class="text-end">Amount</th></tr></thead>
                    <tbody>
            ...
            <div class="table-responsive">
                <table class="table table-sm align-middle mb-0 small">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td class="font-monospace">{{ $app->invoice->invoice_no ?? ('#'.$app->supplier_invoice_id) }}</td>
                            <td class="text-end font-monospace text-success">{{ number_format($app->applied_amount,2) }}</td>
                            @if($apCreditNote->isApproved())
                            <td class="text-end">
                                @can('finance.ap-credit-notes.edit')
                                <form method="POST" action="{{ route('finance.ap-credit-notes.applications.destroy', [$apCreditNote, $app]) }}" onsubmit="return confirm('Remove this application?')">
                        ...
                        @empty
                        <tr><td colspan="3" class="text-center text-muted py-3 fst-italic">Not applied to any bill yet.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
            @if($apCreditNote->isApproved() && $apCreditNote->unapplied > 0 && $pendingInvoices->isNotEmpty())
            @can('finance.ap-credit-notes.edit')
            <div class="card-footer bg-transparent">
                <form method="POST" action="{{ route('finance.ap-credit-notes.applications.store', $apCreditNote) }}" class="row g-2 align-items-end">
                    @csrf
                    <div class="col-md-6">
...
(function () {
    ...
    sel.addEventListener('change', function () {
        ...
        const unapplied = {{ $apCreditNote->unapplied }};
