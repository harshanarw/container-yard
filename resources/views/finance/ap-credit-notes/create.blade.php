            ...
            <table class="table table-sm align-middle mb-0 small">
                ...
                @if($prefill ?? null)
                    <tr>
                        ...
                        <td>
                            <select name="lines[0][expense_account_id]" class="form-select form-select-sm">
                                <option value="">— Default expense —</option>
                                @foreach($expenseAccounts as $a)
                                <option value="{{ $a->id }}" {{ ($prefill['expense_account_id'] ?? null) == $a->id ? 'selected' : '' }}>{{ $a->code }} — {{ $a->name }}</option>
                                @endforeach
                            </select>
                        </td>
