...
@section('title', 'Edit ' . $supplierInvoice->invoice_no)

@section('breadcrumb')
    <li class="breadcrumb-item">Finance</li>
    <li class="breadcrumb-item"><a href="{{ route('finance.ap.invoices.index') }}">Supplier Invoices</a></li>
    <li class="breadcrumb-item"><a href="{{ route('finance.ap.invoices.show', $supplierInvoice) }}">{{ $supplierInvoice->invoice_no }}</a></li>
...
<form method="POST" action="{{ route('finance.ap.invoices.update', $supplierInvoice) }}" id="invoiceForm">
    @csrf
    @method('PUT')
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-6">
                            ...
                            <input type="text" name="supplier_invoice_no" class="form-control form-control-sm"
                                value="{{ old('supplier_invoice_no', $supplierInvoice->supplier_invoice_no) }}" placeholder="Ref / Bill number">
                        </div>
                        <div class="col-6">
                            ...
                            <input type="date" name="supplier_bill_date" id="supplierBillDate" class="form-control form-control-sm"
                                value="{{ old('supplier_bill_date', $supplierInvoice->supplier_bill_date?->format('Y-m-d')) }}">
                        </div>
                        <div class="col-12">
                            <label class="form-label small">Notes</label>
                            <textarea name="notes" class="form-control form-control-sm" rows="2">{{ old('notes', $supplierInvoice->notes) }}</textarea>
                        </div>
                        @php
                            $jobName = function ($j) {
                                $p = [];
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-6">
                            ...
                            <input type="date" name="due_date" id="dueDateInput" class="form-control form-control-sm"
                                value="{{ old('due_date', $supplierInvoice->due_date?->format('Y-m-d')) }}">
                        </div>
                        <div class="col-6">
    ...
    <div class="d-flex gap-2">
        ...
        <a href="{{ route('finance.ap.invoices.show', $supplierInvoice) }}" class="btn btn-outline-secondary btn-sm">Cancel</a>
    </div>
</form>
...
@php
    ...
    // Seed lines: old() after failed validation, otherwise existing invoice lines.
    $oldLines = old('lines', $supplierInvoice->lines->map(fn($l) => [
        'description'        => $l->description,
        'expense_account_id' => $l->expense_account_id,
        'amount'             => $l->amount,
        'charge_code_id'     => $l->charge_code_id,
        'tax_code_id'        => $l->tax_code_id,
        'tax1_rate'          => $l->tax1_rate,
        'tax2_rate'          => $l->tax2_rate,
        'yard_job_id'        => $l->yard_job_id,
    ])->all());

    // Supplier's stored payment terms (to pre-select Credit Terms on load).
    $supplierTerms = $supplierInvoice->supplier->ap_payment_terms ?? '';
@endphp
...
(function () {
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // Pre-select Credit Terms from the supplier's payment terms.
        const supplierTerms = @json($supplierTerms);
        if (supplierTerms) {
            document.getElementById('creditTermsSelect').value = supplierTerms;
        }

        // Seed existing line rows (must run after DOMContentLoaded so s2CodeResult is ready).
        const seed = @json(array_values($oldLines));
        if (seed.length) seed.forEach(line => addRow(line)); else addRow();

        // ── Job costing wiring ────────────────────────────────────────────────
        jQuery('#supplierSelect').on('change', refreshAllJobSelects);
        jQuery('#jobShowAll').on('change', refreshAllJobSelects);
        jQuery('#yard_job_id').on('change', function () {
            const v = this.value;
            if (v) jQuery('select.job-line').each(function () { jQuery(this).val(v).trigger('change.select2'); });
        });
        window.initJobSelect(jQuery('#yard_job_id'));
