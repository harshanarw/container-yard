...
<div class="page-header d-flex justify-content-between align-items-center flex-wrap gap-2 mb-3">
    <div>
        ...
        <p class="text-muted small mb-0">Outstanding receivables by customer — as of {{ \Carbon\Carbon::parse($asOf)->format('d M Y') }} · aged by {{ $ageBy === 'invoice_date' ? 'invoice date' : 'due date' }} · buckets in {{ $base }}@if($currencyFilter) · currency: {{ $currencyFilter }}@endif</p>
    </div>
...
<div class="card content-card">
    <div class="card-body text-center text-muted py-5">
        ...
        No outstanding receivables as of {{ \Carbon\Carbon::parse($asOf)->format('d M Y') }}@if($currencyFilter) in {{ $currencyFilter }}@endif.
    </div>
</div>
@else
<div class="card content-card">
    <div class="card-header bg-transparent py-2 d-flex justify-content-between align-items-center">
        <strong class="small">Customer Detail</strong>
            ...
            <tbody>
                ...
                <tr class="{{ $loop->first ? 'border-top border-2' : '' }}">
                    ...
                    <td class="font-monospace">
                        @php
                            $invRoute = match($inv['type']) {
                                'storage'          => route('billing.show', $inv['id']),
                                'storage-handling' => route('billing.storage-handling.show', $inv['id']),
                                'reefer'           => route('billing.reefer.show', $inv['id']),
                                'repair'           => route('repair-invoices.show', $inv['id']),
                                default            => null,
                            };
                        @endphp
                        @if($invRoute)
                        <a href="{{ $invRoute }}" class="text-decoration-none">{{ $inv['invoice_no'] }}</a>
                        @else
                        {{ $inv['invoice_no'] }}
                        @endif
                    </td>
                    ...
                    <td>
                        <span class="badge bg-secondary-subtle text-secondary">{{ $inv['type_label'] }}</span>
                    </td>
