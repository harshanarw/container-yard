                            ...
                            <tbody id="lineItems">
                                ...
                                <tr class="estimate-line">
                                    <td class="ps-3">
                                        ...
                                        <input type="hidden" name="line_items[{{ $i }}][washing_tariff_id]" value="{{ $item->washing_tariff_id }}">
                                        <input type="hidden" name="line_items[{{ $i }}][wash_scope]"         value="{{ $item->wash_scope }}">
...
(function () {
        ...
        tbody.insertAdjacentHTML('beforeend', `
            <tr class="estimate-line js-new-line">
                <td class="ps-3">
                    ...
                    <input type="hidden" name="line_items[${i}][washing_tariff_id]" value="">
                    <input type="hidden" name="line_items[${i}][wash_scope]"         value="">
    ...
    (function () {
        ...
        const EST_CUSTOMER = '{{ $estimate->customer_id }}';
        const EST_SIZE     = '{{ $estimate->size }}';
        ...
        // Append a blank row via the existing Add-Line flow, then populate it.
        function insertWashingLine(r) {
            document.getElementById('addLine').click();
            const tbody = document.getElementById('lineItems');
            const row   = tbody.lastElementChild.previousElementSibling; // main row (bd-row is last)
            if (!row) return;

            const set = (sel, val) => { const el = row.querySelector(sel); if (el) el.value = val; };
            set('.comp-desc', r.label);
            set('.qty', '1');
            set('.unit-price', Number(r.unit_price).toFixed(2));
            set('[name$="[repair_type]"]', 'clean_and_treat');
            set('[name$="[washing_tariff_id]"]', r.washing_tariff_id ?? '');
            set('[name$="[wash_scope]"]', r.wash_scope ?? '');

            // Charge/tax are Select2 — set the native value then refresh the widget
            // without firing the app change handlers (recalculate reads the native
            // selectedOption datasets regardless).
            ...
            setSelect('[name$="[charge_code_id]"]', r.charge_code_id);
            setSelect('.tax-code-sel', r.tax_code_id);
        }

        function addSelected() {
            Array.from(document.querySelectorAll('.wash-pick:checked')).forEach(el => {
                const r = resolved[el.value];
                if (r && r.found) insertWashingLine(r);
            });
            recalculate();
