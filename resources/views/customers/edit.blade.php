                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="text" name="tin_number" class="form-control"
                                   ...
                                   maxlength="20"
                                   value="{{ old('tin_number', $customer->tin_number) }}">
                    ...
                    <div class="row g-3">
                        <div class="col-md-6">
                            ...
                            <div class="input-group">
                                ...
                                <input type="number" name="ap_credit_limit" class="form-control"
                                       placeholder="0.00" min="0" step="0.01"
                                       value="{{ old('ap_credit_limit', $customer->ap_credit_limit) }}">
                            </div>
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="ap_payment_terms" class="form-select">
                                ...
                                <option value="net45" {{ old('ap_payment_terms', $customer->ap_payment_terms)=='net45'?'selected':'' }}>Net 45 Days</option>
                                <option value="net60" {{ old('ap_payment_terms', $customer->ap_payment_terms)=='net60'?'selected':'' }}>Net 60 Days</option>
                            </select>
...
(function () {
        ...
        fetch(districtsUrl + '?state_id=' + stateId)
            ...
            .then(districts => {
                ...
                districts.forEach(d => {
                    ...
                    if (preselectDistrictId && d.id == preselectDistrictId) opt.selected = true;
        ...
        fetch(statesUrl + '?country_id=' + countryId)
            ...
            .then(states => {
                if (states.length) {
                    ...
                    initSelect2State();
                    if (preselectStateId) loadDistricts(preselectStateId, preselectDistrictId);
                } else {
                    stateDropWrap.style.display = 'none';
                    stateFreeWrap.style.display = '';
                }
    ...
    // On page load: trigger AJAX load for the customer's country, pre-selecting saved values
    const initCountryId   = document.getElementById('countrySelect').value;
    const initStateId     = @json(old('state_id', $customer->state_id));
    const initDistrictId  = @json(old('district_id', $customer->district_id));
    if (initCountryId) loadStates(initCountryId, initStateId, initDistrictId);
})();
</script>
@endpush
